...
async function main({ pool }) {
  ...
  try {
    while (true) {
      ...
      let woke = false;
      const wakeReason = moreBlocks
        ? "unfinished addNewHeaders"
        : await Promise.race([
...
async function applyJobs({ pool, provider }) {
  ...
  for (const { jobId, lastBlockNumber } of jobProgress) {
    ...
    const length = Math.min(2000, localHead.blockNumber - minBlock + 1);
    const maxBlock /* inclusive */ = minBlock + length - 1;
  ...
  if (errors.length === 0) return { type: "OK" };
